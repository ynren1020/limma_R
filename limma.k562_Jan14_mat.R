...
##protein_coding file from hg38_testsort.gtf (/home/yren/project/k562/hg38_testsort.protein_coding.txt)##
...
dge2vs1<-read.delim("parental.degs.txt")
dge2vs1sub<-dge2vs1[dge2vs1$adj.P.Val<0.0005&abs(dge2vs1$logFC)>2,]
dge2vs1sub<-dge2vs1sub[-grep("XLOC_",row.names(dge2vs1sub),ignore.case=FALSE,fixed=TRUE),]
row.names(dge2vs1sub)<-substring(row.names(dge2vs1sub),1,15)
dge2vs1sub$geneid<-row.names(dge2vs1sub)
...
ensembl = useEnsembl(biomart="ensembl",dataset="hsapiens_gene_ensembl")
#filters = listFilters(ensembl)
genesym3vs2<-getBM(attributes=c('ensembl_gene_id', "hgnc_symbol"), filters = "ensembl_gene_id", values=rownames(dge2vs1sub), mart=ensembl)

#known_3vs2$geneid<-rownames(known_3vs2)
genesym3vs2$geneid<-genesym3vs2$ensembl_gene_id
dge2vs1subfinal<-dplyr::full_join(dge2vs1sub,genesym3vs2,by="geneid")
#known_2vs1final$genename<-known_2vs1final$hgnc_symbol
#known_2vs1final<-known_2vs1final[,c(10,13,1:9)]
#allDGE_2vs1<-rbind(known_2vs1final,novel_2vs1)
...
k562knownfinal_matU2<-k562knownfinal_matU[row.names(k562knownfinal_matU)%in%dge2vs1subfinal$hgnc_symbol,]

for (i in 1:nrow(k562knownfinal_matU2)){
    if (row.names(k562knownfinal_matU2)[i]=="CASC9") {row.names(k562knownfinal_matU2)[i]<-"000000000000000000000000CASC9.CASC9.CASC9.CASC9"}
    else if (row.names(k562knownfinal_matU2)[i]=="LINC00989") {row.names(k562knownfinal_matU2)[i]<-"000000000000000000000000000000000LINC00989.LINC00989"}
    else row.names(k562knownfinal_matU2)[i]<-i
}
...
for(i in 1:nrow(k562knownfinal_matU2)){
rownames[i]<-ifelse(row.names(k562knownfinal_matU2)[i]=="CASC9"|row.names(k562knownfinal_matU2)[i]=="LINC00989",T,F)
}

H<-pheatmap(k562knownfinal_matU2,
            show_colnames = T,
            show_rownames = T,
            fontsize_row=5,
            fontsize_col=12,
...
##use known2vs1lncrna_negative_both dataset##
lncrna2vs1negboth<-read.delim("known2vs1lncrna_negative_both.csv",sep=",")
#knownall<-read.delim("knowalllncrnadiff.csv",sep=",")

lncrnaneg1<-read.delim("known2vs1lncrna_negative_1.csv",sep=",")
lncrnaneg1sub<-lncrnaneg1[,c("logFC","hgnc_symbol")]
.rowNamesDF(lncrnaneg1sub,make.names=TRUE)<-lncrnaneg1sub$hgnc_symbol
lncrnaneg1sub$logFC<-abs(lncrnaneg1sub$logFC)
lncrnaneg1sub<-lncrnaneg1sub[order(-lncrnaneg1sub$logFC),]
lncrnaneg1sub$hgnc_symbol<-NULL
lncrnaneg1sub_mat<-as.matrix(lncrnaneg1sub)
#lncrnaneg1suborder<-lncrnaneg1sub[order(lncrnaneg1sub$logFC),]


png(filename = "logFC.heatmap.red6.png",width = 3, height = 6,units='in',res = 400,pointsize = 4)
