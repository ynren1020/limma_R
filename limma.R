...
src.data <- read.delim("count.txt", row.names="Geneid", check.names=FALSE);
...
##2019-02-01##
##read si025.degs.txt for rgt-TDF analysis##
vcap<-read.delim("si025.degs.txt",sep="\t",row.names = TRUE)
vcap$geneid<-row.names(vcap)
vcapgenelist<-dplyr::filter(vcap,abs(logFC)>2)
vcapgenelist<-vcapgenelist$geneid
write.table(vcapgenelist,file="vcapgenelist.txt",sep="\t",row.names = FALSE,col.names = FALSE,quote = FALSE)
